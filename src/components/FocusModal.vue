...
const emit = defineEmits(['close', 'move', 'toggle-fav'])
...
<template>
  <div class="focus-overlay" @click.self="emit('close')">
    ...
    <div v-if="!previewable" class="focus-card">
      ...
      <div class="focus-nav">
        <button class="nav-btn" :disabled="focusIndex === 0" @click="emit('move', -1)">← 前へ</button>
        <button class="nav-btn" :disabled="focusIndex === total - 1" @click="emit('move', 1)">次へ →</button>
      </div>
      <span class="focus-back-btn" @click="emit('close')">[ グリッドに戻る ]（Escキー）</span>
    </div>
    ...
    <div v-else class="focus-preview">
      ...
      <div class="preview-footer">
        <button class="nav-btn" :disabled="focusIndex === 0" @click="emit('move', -1)">← 前へ</button>
        <span class="focus-back-btn" style="margin:0" @click="emit('close')">[ グリッドに戻る ]（Escキー）</span>
        <button class="nav-btn" :disabled="focusIndex === total - 1" @click="emit('move', 1)">次へ →</button>
      </div>
    </div>
  </div>
